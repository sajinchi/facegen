...
const SubscriptionViewPage = () => {
  ...
  return (
    ...
    <table className="align-middle text-left w-full ">
      ...
      <tbody>
        {Boolean(subscription.status == StoreStatusEnum.SUCCESS) && (
          <>
            {subscription.data.map((pro, index) => {
              return (
                <tr
                  key={index}
                  className="font-medium odd:bg-white even:dark:bg-tablecolor1 h-14 "
                >
                  <td className="px-6 py-3">{pro.package.name}</td>
                  <td className="px-6 py-3">{pro.id}</td>
                  <td className="px-6 py-3">{pro.status}</td>
                  <td className="px-6 py-3">{pro.type}</td>
                  <td className="px-6 py-3">{pro.subscription_id}</td>
                  <td className="px-6 py-3">
                    <span className="flex flex-row items-center space-x-3 text-xl">
                      <div>
                        <Link
                          href={{
                            pathname: "./subscriptionSingle",
                            query: "id=" + pro.id,
                          }}
                        >
                          <button>
    ...
    {Boolean(subscription.status == StoreStatusEnum.IDLE) && <></>}
    {Boolean(subscription.status == StoreStatusEnum.LOADING) && (
      <>
        <div className="py-5">
    ...
    {Boolean(subscription.status == StoreStatusEnum.ERROR) && (
